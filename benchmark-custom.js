...
async function customBench(name, task) {
  ...
  console.log(name, (1e3 / geoMean(durs)).toFixed(1) + ' ops/s', (Math.max(...rss) / 1024 / 1024).toFixed(1) + ' peak RSS (MiB)');
}

await customBench('udsv', () => useUdsvDeep(data))
await customBench('csv42', () => useCsv42(data))
await customBench('papaparse+flat', () => usePapaParseDeep(data))
